...
$query="SELECT USER,PASSWORD,ADMINISTRATOR FROM $_Tables[0]";
...
while($Access=mysqli_fetch_array($result,MYSQLI_ASSOC)){ 
	if($User==$Access['USER'] && $Passwordmd5==$Access['PASSWORD'] && $Admin==$Access['ADMINISTRATOR']){
	...
	elseif($User==$Access['USER'] && $Passwordmd5==$Access['PASSWORD'] && $UserCond==$Access['ADMINISTRATOR']){
		$toinsert="INSERT INTO $_Tables[4]
				(USER_ACCESS,IP_ACCESS,DATE_ACCESS,HOURS_ACCESS)
				VALUES
		('$User','$Remoute_Address','$_Date_Access','$Hour_Access')";
		...
		sleep(2);
		print("Accesso");
		header("location:../Pannel_Users/Pannel_Users.php");
		}
}
